...
class Validators {
    ...
    static minLength(len) {
        return value => value.toString().length < len ? `The number of characters must be greater than ${len}` : null;
    }

    static maxLength(len) {   
        return value => value.toString().length > len ? `The number of characters must be less than ${len}` : null;
    }

    static minAndMaxLength(min, max) {
        return value => (value.toString().length > max) || (value.toString().length < min) ? `The number of characters must be greater than ${min} and less than ${max}` : null;
    }

  	static minNumber(min) {
		return value => value < min ? `The number must be greater than ${min}` : null;
	}

  	static maxNumber(max) {
    	return value => value > max ? `The number must be less than ${max}` : null;
  	}

  	static minAndMaxNumbers(min, max) {
        return value => value > max || value < min ? `The numerical range must be greater than ${min} and less than ${max}` : null;
    }
